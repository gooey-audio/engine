...
pub struct SnareConfig {
    pub snare_frequency: f32, // Base tonal frequency (150-250Hz typical)
...
impl SnareConfig {
    pub fn new(
        ...
        Self {
            snare_frequency: snare_frequency.max(100.0).min(400.0), // Reasonable snare range
            tonal_amount: tonal_amount.clamp(0.0, 1.0),
            noise_amount: noise_amount.clamp(0.0, 1.0),
            crack_amount: crack_amount.clamp(0.0, 1.0),
            decay_time: decay_time.max(0.01).min(2.0), // Short impulse range
    ...
    pub fn default() -> Self {
        Self::new(200.0, 0.3, 0.8, 0.6, 0.15, 0.4, 0.8) // Short decay for impulse
    }

    pub fn crispy() -> Self {
        Self::new(220.0, 0.2, 0.9, 0.8, 0.12, 0.3, 0.85)
    }

    pub fn deep() -> Self {
        Self::new(180.0, 0.5, 0.7, 0.4, 0.25, 0.6, 0.9)
    }

    pub fn tight() -> Self {
        Self::new(240.0, 0.4, 0.8, 0.7, 0.08, 0.2, 0.8) // Very short for tight sound
    }
}
...
pub struct SnareDrum {
    ...
    pub tonal_oscillator: Oscillator,  // Tonal component
    pub noise_oscillator: Oscillator,  // Main noise body
    pub crack_oscillator: Oscillator,  // High-frequency crack/snap
...
impl SnareDrum {
    ...
    pub fn with_config(sample_rate: f32, config: SnareConfig) -> Self {
        let mut snare = Self {
            ...
            noise_oscillator: Oscillator::new(sample_rate, config.snare_frequency),
            crack_oscillator: Oscillator::new(sample_rate, config.snare_frequency * 15.0),
            pitch_envelope: Envelope::new(),
            base_frequency: config.snare_frequency,
            pitch_start_multiplier: 1.0 + config.pitch_drop * 1.5, // Start higher for pitch drop
            is_active: false,
        };
    ...
    fn configure_oscillators(&mut self) {
        ...
        self.tonal_oscillator.set_adsr(ADSRConfig::new(
            ...
            config.decay_time * 0.7, // Medium decay for tonal component
            0.05,                    // Very low sustain for impulse
            config.decay_time * 0.3, // Short release
        ));

        // Noise oscillator: Snare noise for main body
        self.noise_oscillator.waveform = Waveform::SnareNoise;
        self.noise_oscillator.frequency_hz = config.snare_frequency;
        self.noise_oscillator
            .set_volume(config.noise_amount * config.volume * 0.8);
        self.noise_oscillator.set_adsr(ADSRConfig::new(
            0.001,                   // Very fast attack
            config.decay_time * 0.9, // Longer decay for noise body
            0.02,                    // Minimal sustain for impulse
            config.decay_time * 0.4, // Short release
        ));

        // Crack oscillator: White noise for high-frequency snap
        self.crack_oscillator.waveform = Waveform::WhiteNoise;
        self.crack_oscillator.frequency_hz = config.snare_frequency * 15.0;
        self.crack_oscillator
            .set_volume(config.crack_amount * config.volume * 0.4);
        self.crack_oscillator.set_adsr(ADSRConfig::new(
            0.001,                    // Very fast attack
            config.decay_time * 0.1,  // Very short decay for crack
            0.0,                      // No sustain for crack
        ...
        self.pitch_envelope.set_config(ADSRConfig::new(
            ...
            config.decay_time * 0.2, // Quick pitch drop
    ...
    pub fn tick(&mut self, current_time: f32) -> f32 {
        ...
        // Apply pitch envelope to tonal oscillator only (noise doesn't need pitch modulation)
    ...
    pub fn set_frequency(&mut self, frequency: f32) {
        self.config.snare_frequency = frequency.max(100.0).min(400.0);
